...
def main():
    ...
    opt = bpop.optimisations.DEAPOptimisation(evaluator,seed=1178, offspring_size=20,  eta=20, mutpb=0.3, cxpb=0.7, hof = tools.ParetoFront())
    #
    if (cp_true ==True):
        pop, hof, log, hst = opt.run(max_ngen=2, cp_filename=cp_file, cp_frequency=1,continue_cp=True)
    else:
        pop, hof, log, hst = opt.run(max_ngen=2, cp_filename='cp.pkl', cp_frequency=1)
