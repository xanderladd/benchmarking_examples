...
def read_gpu_logs(fn):
    ...
    lines = lines[:500000] # limit to .5 mil lines
...
def time_to_50(abstart, start_end_pairs):
    try:
        ...
        res = (float(start_end_pairs[-1][1]) - float(abstart) / len(start_end_pairs)) * 50
...
#             axs.bar(idx/1.5, total_time , width, label='Full Runtime',color="black")
            axs.bar(plot_idx/1.5, mean_deap , width, yerr=std_deap, label='Runtime on DEAP', color="orange",edgecolor='black', linewidth=1.5)
...
def time_to_50(abstart, start_end_pairs):
    try:
        ...
        print(f"WARNING only {len(start_end_pairs)} start end pairs")
        end = start_end_pairs[-1][1]
    return float(end) - float(abstart)
...
def generate_result_table(nCpus,nodes,pops, sfs, stims, versions, path, title=None, how='vanilla'):
    df = pd.DataFrame(columns=['Nodes','Total Cpus', 'Total Gpus',\
                               ...
                               'Runtime', 'Runtime Stddev', 'FOM', 'FOM Std Dev','GPU Utilization', 'Mean Eval Time', 'Std Eval Time', 'Mean Sim Time', 'Std Sim Time','DEAP time', 'DEAP Time Std Dev', 'Time to 50 Gen', 'Mean Gen Size', 'Std Gen Size'])
    if title:
    ...
    for idx,(node,pop,nCpu,stim, sf, vers) in enumerate(zip(nodes,pops,nCpus, stims, sfs, versions)):
        ...
        df.loc[idx] = [int(node),int(nCpu)*int(node), nGpus*int(node), \
                     int(pop),int(stim), int(sf), float(mean_runtime), float(std_runtime), fom_mean, fom_dev, pct_util, mean_eval, std_eval, mean_neuroGPU, std_neuroGPU,  mean_deap, std_deap, total_time, mean_gen_size, std_gen_size]
...
def write_all_files(dest, srcs, combine=True):
    # set combine logs to false for now
    combine = False
    if combine:
        with open(dest, 'r') as prevfile:
            prev_lines =  prevfile.readlines()
    else:
        prev_lines = []
    with open(dest, 'w') as outfile:
        outfile.writelines(prev_lines)
        for fname in srcs:
...
def merge_experiments(src, dest, version, path):
    ...
    if os.path.isfile(log_path) and os.path.isfile(old_log) :
        write_all_files(log_path, [log_path, old_log], combine=True)
    elif  os.path.isfile(old_log):
        #assert int(version) == 1, "no master log and version is not 1?"
        write_all_files(log_path, [old_log], combine=False)
    else:
